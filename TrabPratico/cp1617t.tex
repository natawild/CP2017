...
\\[\blanklineskip]%
\>[B]{}\mbox{\onelinecomment  prop_inv_correctness x = inv x n - (1 / x) < 0.1}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
part:: (a -> Bool) -> [a] -> ([a], [a])
part p []                = ([],[])
part p (h:t) \ensuremath{\Varid{p}\;\Varid{h}\mathrel{=}\mathbf{let}\;(\Varid{s},\Varid{l})\mathrel{=}\Varid{part}\;\Varid{p}\;\Varid{t}\;\mathbf{in}\;(\Varid{h}\mathbin{:}\Varid{s},\Varid{l})} otherwise = let (s,l) = part p t in (s,h:l)
